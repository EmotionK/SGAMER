...
def read_config(conf_name):
    ...
    with open(conf_name) as IN:
        ...
        config['metapath_dict'] = dict() #{edge:number}
        config['metapath_to_edge_dict'] = dict() #{number:edge}
...
class Sampler(object):
    def init(self, config, Hin_dict,mappings, data_info):
        self.metapath_to_edge_type_to_edge_index_record = dict() #{0:{'a:p:a' : 0到'a:p'边的数量的随机数}}
        self.metapath_to_edge_type_index_record = dict() #{0:0}
        self.metapath_to_edge_type = config['metapath_to_edge_dict']
        for metapath_idx in self.metapath_to_edge_type : #每个位置的三元素序列 {0: ['a:p:a', 'p:a:p'], 1: ['p:t:p', 't:p:t'], 2: ['p:p:t', 'p:t:p', 't:p:p'], 3: ['a:p:t', 'p:a:p', 'p:t:p', 't:p:a'], 4: ['p:p:p'], 5: ['a:p:p', 'p:a:p', 'p:p:a']}
            ...
            for edge_type in self.metapath_to_edge_type [metapath_idx]:
                sub_edge = ':'.join(edge_type.split(':')[:2]) #a:p
        ...
        self.idx_to_edge_types = config['edge_types'] #['a:p:t','p:p:t','t:p:p','p:a:p','t:p:a','t:p:t','p:t:p','p:p:p','a:p:p','p:p:a','a:p:a']
        self.edge_types_to_idx = {edge_type:idx for idx, edge_type in enumerate(self.idx_to_edge_types)}
        self.idx_to_node_types = config['nodes'] #['a','p','t']
        self.node_types_to_idx = {node_type: idx for idx, node_type in enumerate(self.idx_to_node_types)}

        self.Hin_dict = Hin_dict #{node_a_type:{node_a_id:{node_b_type:[node_b_id]}}}
        '''
        {
        out_mapping:{node_type:{len(self.in_mapping[type]):node_id}}
        in_mapping:{node_type:{node_id:len(self.in_mapping[type])}}
        }
        '''
        self.mappings = mappings

        self.data_info = data_info #_info_dict.p
    ...
    def edge_list_preparison(self, hin_file): #cora.hin
        edge_type_list = dict() #{edge_type:[(node_a_id,node_b_id)]}
        edge_type_list_lim = dict() #{edge_type:该边类型的数量}
        lines = open(hin_file,'r').readlines()
        self.num_edge = len(lines)
        for edge in lines: # p:15142 a:238 1 p:a
            try:
        ...
        self.edge_type_list = edge_type_list #{edge_type:[(node_a_id,node_b_id)]}
        self.edge_type_list_lim = edge_type_list_lim #{edge_type:该边类型的数量}
