...
def get_instance_paths(path_file) -> list:
    ...
    paths_list = []
    for user_item_paths_relation in user_item_paths_relation_uibi:
        #print(f'user_item_paths_relation:{user_item_paths_relation}')
        ui, pathnum, path_list_str = user_item_paths_relation.strip().split('\t', 2)
        #print(f'ui, pathnum, path_list_str:{ui},{pathnum},{path_list_str}')
        path_list = path_list_str.split('\t')  # pathnum 是 list的长度
                ...
                if (u,i) in ele.keys():
                    if (u,i) not in this_user_ui_instances_embs.keys():
                        ...
                        for list_ele in ele[(u,i)]:
                            #print(this_user_ui_instances_embs[(u,i)])
                            # print(f'this_user_ui_instances_embs:{type(this_user_ui_instances_embs[(u,i)])}')
                            if type(this_user_ui_instances_embs[(u,i)])==torch.Tensor:
                                this_user_ui_instances_embs[(u, i)] = this_user_ui_instances_embs[(u,i)].tolist()
            ...
            if (i1, i2) in ii_metapath_emb.keys():
                if (i1, i2) not in this_user_ii_instances_embs.keys():
                    ...
                    for list_ele in ii_metapath_emb[(i1, i2)]:
                        if type(this_user_ii_instances_embs[(i1, i2)])==torch.Tensor:
                            this_user_ii_instances_embs[(i1, i2)] = this_user_ii_instances_embs[(i1, i2)].tolist()
